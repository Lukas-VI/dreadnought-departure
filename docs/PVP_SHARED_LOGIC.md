...
| 计时 | 玩家侧 + AI 侧倒计时 | 服务端授时（start/end 时间戳），客户端按服务器时钟展示并在截止时提前提交 |
| 地图 | 从 `export/maps` 加载 | 房主经战役选图上传，房间缓存，对方下载 |

## 4. 当前边界与后续选项

- 视野/鱼雷阶段按地图开关自动跳过并记录；交互式玩法留待后续，鱼雷暂按既定“后推”安排。
- ShipData 已共用：客户端上传数值表，服务端不再内嵌维护。
- 单纵阵/堆叠判定已与客户端一致（同速同向、逐格链、堆叠并入、旧编队保留）。
- 本地与 PvP 共用 `ShipCommandIntent`：本地结算消费同一构建器，PvP 通过 wire 结构提交。
- 移动已引入速力表；单纵阵跟随舰重放前舰上一阶段完整轨迹，逐段转向跟随。
