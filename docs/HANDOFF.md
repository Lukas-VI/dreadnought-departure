...
> 最后提交：`dada6ec feat: 指挥值随损伤减少并接入PV评分系统`
...
- 已提交：`dada6ec`（指挥值随损伤减少、双方 CP 与 PV 评分）、`fd57304`（单纵阵转向格立即转向与转向补间动画）、`bf754a2` / `a3d46ae`（贪吃蛇转向与文档同步）及更早的编辑器关卡设置系列。
- 本次提交包含指挥/CP/PV 相关源码与三份文档；工作区中 `Ships/*/*_data.tres`、`export/maps/1.json` 与未跟踪的 `export/maps/4.json` 是用户/编辑器产物，未纳入提交。
