...
> 最后提交：`42b6ca9 fix: 单纵阵支持多转向点轨迹与无移动阶段转向`
...
- 已提交：`42b6ca9`（单纵阵多转向点轨迹与无移动阶段转向）、`fa45e6f`（单纵阵组成/切断提示与列表分组）、`dada6ec`（指挥值随损伤减少、双方 CP 与 PV 评分）、`fd57304`（单纵阵转向格立即转向与转向补间动画）、`bf754a2` / `a3d46ae`（贪吃蛇转向与文档同步）及更早的编辑器关卡设置系列。
- 本次提交包含单纵阵轨迹/提示/分组相关源码、新脚本 uid 与三份文档；工作区中 `Ships/*/*_data.tres`、`export/maps/1.json` 与未跟踪的 `export/maps/4.json` 是用户/编辑器产物，未纳入提交。
