...
| CP / 指挥值 / PV | 本地规则，动作消耗 CP | 服务端扣 CP 并重算指挥值/PV；编队整组优惠尚未对齐 | 基本一致 |
...
- 编队 CP 优惠：单机整条单纵阵变速/转向只扣 1 CP；PvP 服务端当前按逐船指令扣 CP。
- 完整冲撞表：双方目前都使用简化冲撞占位（1D10≤2 + 简易 A3 损伤），完整 A3 表待补。
- 断线重连：PvP 尚未做战斗中途断线恢复与重连。

## 6. 结论

输入、指令意图、速力表、堆叠、单纵阵轨迹、完整炮击、移动阻挡、CP 消耗、延迟降速、主动权初设、服务器授时、胜负结算与房间生命周期已收敛为同一套语义；主要剩余差异集中在视野/鱼雷交互阶段与编队 CP 优惠。
