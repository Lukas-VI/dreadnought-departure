...
| 视野/照明 | 仅夜战进入 `ReconLighting` | 未进入；`recon` 仅保留在 `PHASE_INDEX` 与客户端映射 |
| 炮击 | `Gunfire` | `gunnery` |
| 鱼雷 | 仅开关启用时进入 `Torpedo` | 未进入；`torpedo` 仅保留在 `PHASE_INDEX` 与客户端映射 |
| 回合结算 | `EndTurn` | `settlement` |

单机流转：`SpeedAdjust → MovePhase1/2/3 →（夜战 ReconLighting）→ Gunfire →（鱼雷开关 Torpedo）→ EndTurn → SpeedAdjust`。

PvP 服务端 `NEXT_PHASE` 当前实际流转：`speed → move1 → move2 → move3 → gunnery → settlement → speed`。每阶段由 `activePlayer` 提交，双方都提交或倒计时归零后推进；`settlement` 结算全灭/回合上限，再回到 `speed` 并交换先手。
...
| CP / 指挥值 / PV | 本地规则，动作消耗 CP | 服务端按损伤镜像重算指挥值/PV，动作尚未扣 CP | 有差异 |
...
- 视野阶段：单机夜战进入 `ReconLighting`；PvP 服务端未进入，`recon` 只有映射。
- 鱼雷阶段：单机按地图开关启用；PvP 服务端未进入，`torpedo` 只有映射。
- 炮击结算：PvP 需要补齐 CP 消耗、弹药、射界、距离与装甲伤害，与 `CombatRulesEvaluator.FireEx` 对齐。
- 移动阻挡：PvP 需要补齐岛屿沉没与冲撞规则，与 `ResolveMoveSteps` 对齐。
- 降速时机：单机在回合结算延迟降速；PvP 当前在速度阶段即时生效。
- 主动权初设：单机读地图 `InitiativeOwner`；PvP 当前开战掷骰并每回合交换。
- CP 消耗：PvP 服务端当前只重算上限，尚未对加速/转向/炮击扣 CP。

## 6. 结论

输入、指令意图、速力表、堆叠、单纵阵轨迹、服务器授时、胜负结算与房间生命周期已收敛为同一套语义；主要剩余差异集中在 PvP 服务端炮击结算、移动阻挡与视野/鱼雷交互阶段。
