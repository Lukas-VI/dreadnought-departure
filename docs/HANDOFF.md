...
- 本轮修正：`DetectLineAhead` 按格子收集同向同速船，同格堆叠视为同一列，堆叠领头与堆叠跟随均可入阵；编队保留逻辑允许同格距离 0。
- 本轮修复：无移动阶段堆叠领头转向时，同格另一艘会同步写入转向待命并播放转向动画，后方跟随舰仍延迟转向。
- 本轮新增：主菜单“船坞”入口与船坞场景；`ShipData` 增加 `Camp / IsUnlocked / Background / Cost` 字段并填入四艘原型舰，左右按钮切换查看角色数据卡。
- 本轮新增：鉴赏模式使用 Tween 动画，全屏黑底遮罩淡入、立绘放大居中，只保留“退出鉴赏”按钮，退出时反向淡出。
...
- 鉴赏模式冒烟：点击“鉴赏模式”后全屏遮罩显示，退出按钮可用；点击退出后遮罩隐藏，流程无报错。
...
- 已提交：`1f4d81d`（船坞鉴赏模式全屏动画）、`910d055`（船坞节点化 UI 与顶栏入口）、`b62cb5a`（船坞角色查看与 ShipData 扩展）、`109764d`（堆叠领头无位移同步转向）、`de15218`（单纵阵纳入同格堆叠与堆叠领头）、`c5a22fa`（堆叠局部侧向轴）、`d1102a5`（堆叠 Z 轴并排）、`16259d1`（同阵营堆叠）、`7e3a7f3`（左右列表 MarginContainer）、`33773c6`（指挥/CP 顶部居中与列表弹性滚动）、`3d80edd`（射界 60° 对齐与侧射材质）、`a9c074f`（几何射界 60°/120°）、`0a73f3e`（射界 60°/120° 修正）、`57f8b5a`（前/侧射界材质）、`e432d8b`（炮击范围按射界区分前后/侧射材质）、`42b6ca9`（单纵阵多转向点轨迹与无移动阶段转向）、`fa45e6f`（单纵阵组成/切断提示与列表分组）、`dada6ec`（指挥值随损伤减少、双方 CP 与 PV 评分）、`fd57304`（单纵阵转向格立即转向与转向补间动画）、`bf754a2` / `a3d46ae`（贪吃蛇转向与文档同步）及更早的编辑器关卡设置系列。
- 本次提交包含鉴赏模式动画、船坞场景/控制器与三份文档；工作区中 `BattleUI.tscn` / `BattleUIController.cs` / `pause_menu.tscn` 的 UI 微调、编辑器重建的 `dock_scene.tscn`、`export/maps/1.json` 与未跟踪的 `export/maps/4.json` 是用户/编辑器产物，未纳入提交。
